...
import { savePost, unsavePost } from '../services/savedservices';

const SavedPost = () => {
  // id is post id over here
  const { id } = useParams();
  const [post, setPost] = useState(null);
  const { homeUser } = useSelector(state => state.auth);
  const [saveButtonText, setSaveButtonText] = useState('Unsave');
  ...
  const handleUnsave = async () => {
    ...
    const { data, status } = await unsavePost(input);
            ...
            <Stack flex={70}>
              <Flex>
                ...
                <VStack flex={50} alignItems={'flex-start'}>
                  ...
                  <Text>Phone: {post.owner[0].phone}</Text>
                </VStack>
              </Flex>
            </Stack>
          </Flex>
